...
void Application::semiKCore(){
	...
	short* nextCnt = new short[m_m];
	memset(nextCnt,0,sizeof(short)*m_m);
	...
	while(update){
		...
		for (int u = 0; u < m_m; ++u){
			...
			// 	printf("vertex decrease %d\n",originUb-ub[u] );
			// 	// printf("vertex[%d]changed:\n",u );
			// 	// printf("originCore:%d,newCore:%d,degree:%d\n",originUb,ub[u],degree );
			// }
